...
def parse_args():
    ...
    parser.add_argument('--material', type=str, default='sand', help='material type')
...
def save_positions_pt(positions, iteration):
    positions = np.split(positions, [len(pts_list[0]), len(pts_list[0])+len(pts_list[1]), len(pts_list[0])+len(pts_list[1])+len(pts_list[2])])
    for i, x in zip(range(4), positions):
        x = scaler.inverse(x)
...
mpm.add_particles(particles=pts_list[2],
                material=MPMSolver.material_stationary)

mpm.add_particles(particles=pts_list[3],
                material=MPMSolver.material_stationary)
